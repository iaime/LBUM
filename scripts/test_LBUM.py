...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        LBUM_testing_data = form_language_model_input(LBUM_testing_data, inference_time=True)
        ...
        all_y_pred = np.stack([LBUM_model.predict([LBUM_testing_data['left_input'], LBUM_testing_data['right_input'], LBUM_testing_data['bnAb']], batch_size=32, verbose=1)[0] for _ in range(10)])
    ...
    for fold in range(5):
        ...
        all_y_pred = np.stack([LBUM_model.predict([LBUM_testing_data['left_input'], LBUM_testing_data['right_input'], LBUM_testing_data['bnAb']], batch_size=32, verbose=1)[1] for _ in range(10)])
        y_pred = all_y_pred.mean(axis=0)
        y_std = all_y_pred.std(axis=0)
